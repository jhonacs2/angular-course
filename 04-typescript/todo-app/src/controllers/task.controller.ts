...
export default class TaskController {
     ...
     */
    init(): void {
        const data: TaskModel[] = toArray(this.getTasks()) as TaskModel[];
        this.tasks = data;
    ...
    addTask(data: string): boolean {
        if (data) {
            ...
            const task: TaskModel = new TaskModel(id, data, currentTime, currentTime, false);
    ...
    checkTask(taskId: number): boolean {
        ...
        try {
            const checkMessage: string = `Your action has been executed! A task was ${
                itemData.status ? '' : 'un'
